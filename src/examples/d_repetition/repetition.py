...
#Intro to a while loop
def display_numbers(num):
    cnt = 0
    
    while cnt < num:
        print(" cnt " + str(cnt) + " cnt + 1 " + str(cnt +1 ) + " num " + str(num))
        cnt = cnt + 1# A statement that makes rhe boolean expression false
        print("cnt after adding 1" + str(cnt))

def sum_of_squares(num):  #sum of squares(3) 1 * 1 + 2 * 2 + 3* 3 = 14
    sum = 0
        ...
        if(lot_number.isnumeric()):
            ...
            if(lot_number != 0):
                while((lot_number < 1 or lot_number > 10)):
                    lot_number = int(input('number must be in range 1 to 10'))
